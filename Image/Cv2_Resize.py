...
def main():
    ...
    # 缩小
    downscale = cv2.resize(img, (100, 100), interpolation=cv2.INTER_NEAREST)
    # 放大
    upscale = cv2.resize(img, (2 * width, 2 * height), interpolation=cv2.INTER_LINEAR)
    # interpolation——插值算法（最近临插值算法cv2.INTER_NEAREST、双线性插值算法cv2.INTER_LINEAR（默认为双线性））
    print(upscale.shape)
    # 设置窗口属性，并显示图片
    cv2.namedWindow("downscale")
    cv2.imshow("downscale", downscale)

    # 设置窗口属性，并显示图片
    cv2.namedWindow("upscale")
